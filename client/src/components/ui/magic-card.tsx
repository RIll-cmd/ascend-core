...
export function MagicCard(props: MagicCardProps) {
  const {
    ...
    backgroundColor,
    gradientSize = 240,
    gradientColor = "rgba(224, 83, 68, 0.12)",
    gradientOpacity = 0.8,
    gradientFrom = "#e05344",
    gradientTo = "#fba170",
    mode = "gradient",
  ...
  const glowFrom = isOrbMode(props) ? (props.glowFrom ?? "#e05344") : "#e05344";
  const glowTo = isOrbMode(props) ? (props.glowTo ?? "#fba170") : "#fba170";
    ...
    <motion.div
      ...
      style={{
        background: useMotionTemplate`
          linear-gradient(${backgroundColor ?? "var(--color-background, rgba(20, 14, 16, 0.95))"} 0 0) padding-box,
          radial-gradient(${gradientSize}px circle at ${mouseX}px ${mouseY}px,
            ${gradientFrom},
            ${gradientTo},
            var(--color-border, rgba(255, 235, 225, 0.08)) 100%
          ) border-box
        `,
      }}
    >
      <div className={cn("absolute inset-px z-20 rounded-[inherit]", innerClassName ?? "bg-background/80")} />
