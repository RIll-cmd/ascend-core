...
export const InputOTPGroup = ({ className, ...props }: SharedProps) => {
  return (
    <ShadcnInputOTPGroup {...props} className={cn("flex gap-2.5 items-center", className)} />
  );
};
...
export const InputOTPSlot = ({
  ...
  return (
    <div className="relative size-12 border-y-6 border-foreground dark:border-ring bg-[#141414]">
      <ShadcnInputOTPSlot
        index={index}
        {...props}
        className={cn(
          "pl-1 size-full text-center text-xl tracking-widest z-0 ring-0 border-transparent text-white",
          font !== "normal" && "retro",
          className
        )}
      />
...
export const InputOTPSeparator = ({ className, ...props }: SharedProps) => {
  return <ShadcnInputOTPSeparator {...props} className={cn("text-neutral-500 font-bold", className)} />;
};
